...
class Snake(object):
    ...
    def move(self, direction):
        if direction == "UP":
            ...
            self.head[0] = (self.head[0] - 20) % self.window_width
        else:  # RIGHT
            self.head[0] = (self.head[0] + 20) % self.window_width
...
class SnakeGame(object):
    def __init__(self, window_width, window_height):
        ...
        self.snake = Snake(self.window_width, self.window_height, self.screen)
    ...
    def run(self):
        while True:
            ...
            # score
            score_surf = pygame.font.SysFont(FONT_STYLE, 40).render(
                f"Score: {len(self.snake.body) - 1}", True, GRAY
            )
    ...
    def game_start(self):
        self.screen.fill(WHITE)
        name_surf = pygame.font.SysFont(FONT_STYLE, 80).render(
            "Greedy Snake", True, BLACK
        )
    ...
    def gen_position(self):
        x = random.randint(0, self.window_width / 20 - 1) * 20
    ...
    def game_over(self):
        ...
        self.screen.blit(
            game_over_surf,
            (
                (self.window_width - game_over_surf.get_width()) / 2,
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        pygame.mixer.music.set_volume(0.2)
                        self.snake = Snake(
                            self.window_width, self.window_height, self.screen
                        )
                        self.direction = "RIGHT"
                        self.coin_position = [300, 300]
                        self.boom_positions = [[200, 200]]
